...
# Panel (a) shows the dos1rate time series and points where detections 
# were made.                   
...
# Panel (b) shows the wavelet power spectrum.
pIdx = np.where(obj.period < 3)[0]
levels = [0.0625, 0.125, 0.25, 0.5, 1, 2, 4, 8, 16]
CS = ax[1].contourf(obj.time, obj.period[pIdx],
                    np.log2(obj.power[pIdx, :]), len(levels),
                    cmap=plt.cm.jet_r)
